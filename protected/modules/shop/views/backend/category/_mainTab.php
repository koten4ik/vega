            ...
            <?
                /*if(Y::app()->params['multiLang'])
                    $this->widget('JuiTabs', array( 'id'=>'lang-tabs',  'tabs'=>array(
                        'Русский'=>$this->renderPartial('_ltab',array('lng'=>'','model'=>$model, 'form'=>$form),true),
                        'English'=>$this->renderPartial('_ltab',array('lng'=>'_l2','model'=>$model, 'form'=>$form),true),
                        //'DE'=>$this->renderPartial('_ltab',array('lng'=>'_l3','model'=>$model, 'form'=>$form),true),
                    ), 'options'=>array( 'disable'=>true  ),  ));
                else*/
                    $this->renderPartial('_ltab',array('lng'=>'','model'=>$model, 'form'=>$form));
            ?>
