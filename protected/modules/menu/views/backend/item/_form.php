<?

$this->registerScript("
    /*$('.elrte').elrte(".Y::elrteOpts(array('height'=>170)).");*/
    $('.form input[type=text]:first').focus();
", CClientScript::POS_END );
?>

<div class="form" style="margin-top: 0px;">

<?php $form=$this->beginWidget('ActiveForm', array(
	'id'=>'article-category-form',
	'enableAjaxValidation'=>false,
)); ?>

    <input type="hidden" id="cat_id" value="<? echo $model->id; ?>" />
    <input type="hidden" id="parent_id"
           name="<? echo $this->modelName.'[parent_id]' ?>"
           value="<? echo $parent_model->id; ?>"
    />

<table width="100%" class="pad" style="" border="0">
    <tr>
        <td style="padding-right: 7px;">

            <div class="row">
                <span>Родительская категория: </span>
                <span id="parent_name" style="font-weight: bold;">
                    <? echo $parent_model->title; ?>
                </span>
            </div>

            <? if(!Y::app()->params['multiLang']){?>
                <div class="row">
                    <?php echo $form->labelEx($model, 'title' , array('style' => '')); ?>
                    <?php echo $form->textField($model, 'title', array('maxlength' => 150, 'style' => 'width:99%')); ?>
                    <?php echo $form->error($model, 'title'); ?>
                </div>
            <?}?>

            <div class="row">
                <?php echo $form->labelEx($model, 'url' , array('style' => '')); ?>
                <?php echo $form->textField($model, 'url', array('style' => 'width:99%')); ?>
                <?php echo $form->error($model, 'url'); ?>
            </div>

            <div class="row">
                <?php echo $form->labelEx($model, 'rule1' , array('style' => '')); ?>
                <?php echo $form->textField($model, 'rule1', array('style' => 'width:99%')); ?>
                <?php echo $form->error($model, 'rule1'); ?>
            </div>

            <?
                if(Y::app()->params['multiLang'])
                    $this->widget('JuiTabs', array( 'id'=>'lang-tabs',  'tabs'=>array(
                        'Русский'=>$this->renderPartial($this->viewDir.'_ltab',array('lng'=>'','model'=>$model, 'form'=>$form),true),
                        'English'=>$this->renderPartial($this->viewDir.'_ltab',array('lng'=>'_l2','model'=>$model, 'form'=>$form),true),
                        //'DE'=>$this->renderPartial($this->viewDir.'_ltab',array('lng'=>'_l3','model'=>$model, 'form'=>$form),true),
                    ), 'options'=>array( 'disable'=>true  ),  ));
                else $this->renderPartial($this->viewDir.'_ltab',array('lng'=>'','model'=>$model, 'form'=>$form));
            ?>

            <div class="row">
                <?php echo $form->labelEx($model, 'rule2' , array('style' => '')); ?>
                <?php echo $form->textField($model, 'rule2', array('style' => 'width:99%')); ?>
                <?php echo $form->error($model, 'rule2'); ?>
            </div>

            <div class="row">
                <?php echo $form->labelEx($model, 'rule3' , array('style' => '')); ?>
                <?php echo $form->textField($model, 'rule3', array('style' => 'width:99%')); ?>
                <?php echo $form->error($model, 'rule3'); ?>
            </div>

            <div class="row">
                <?php echo $form->labelEx($model, 'menu_id' , array('style' => '')); ?>
                <?php echo $form->textField($model, 'menu_id', array('style' => 'width:99%')); ?>
                <?php echo $form->error($model, 'menu_id'); ?>
            </div>

            <div class="row">
                <?php echo $form->labelEx($model, 'smenu_id' , array('style' => '')); ?>
                <?php echo $form->textField($model, 'smenu_id', array('style' => 'width:99%')); ?>
                <?php echo $form->error($model, 'smenu_id'); ?>
            </div>

            <div class="row">
                <?php echo $form->labelEx($model, 'css_class' , array('style' => '')); ?>
                <?php echo $form->textField($model, 'css_class', array('style' => 'width:99%')); ?>
                <?php echo $form->error($model, 'css_class'); ?>
            </div>

        </td>
        <td width="250px" style="padding-left: 10px;">


            <div class="row" style="padding: 5px 0px;">
                <?php echo $form->checkBox($model,'published'); ?>
                <?php echo $form->labelEx($model,'published', array('class'=>'after_cbox')); ?>
                <?php echo $form->error($model,'published'); ?>
            </div>
            <div class="row">
                <? $this->widget('FieldFile',
                    array('field'=>'image', 'tmb_num'=>1, 'form'=>$form,'model'=>$model, 'local_allow'=>1))?>
            </div>
            <div class="row">
            </div>
        </td>
    </tr>

</table>

<?php $this->endWidget(); ?>

</div><!-- form -->
