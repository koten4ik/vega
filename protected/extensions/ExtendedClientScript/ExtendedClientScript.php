...
class ExtendedClientScript extends CClientScript
{
		...
		if ($this->combineCss)
		{
			if (count($this->cssFiles) !== 0)
			{
				foreach ($this->cssFiles as $url => $media)
					if(!$this->isRemoteFile($url) && !in_array($url, $this->excludeFiles))
	...
	*/
    ...
    public  function buildCommonJS($files, $filename)
    {
        $combinedFile = '';
        foreach ($files as $file) {
            $fileContents = file_get_contents($file);
            $fileContents = $this->minifyJs($fileContents);
            $combinedFile .= '/*----'.$file.'-----*/'.$fileContents;
        }
        file_put_contents($filename, $combinedFile);
    }

    public  function buildCommonCss($files, $filename)
    {
        $combinedFile = '';
        foreach ($files as $file) {
            $fileContents = file_get_contents($file);
